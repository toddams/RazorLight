...
namespace RazorLight.ViewFeatures
{
	...
	public class ModelExplorer
	{
		...
		private ModelExplorer[] PropertiesInternal
		{
			get
			{
				if (_properties == null)
				{
					...
					var FastPropertySetters = FastPropertySetter.GetProperties(ModelType);

					_properties = new ModelExplorer[properties.Count];
					for (var i = 0; i < properties.Count; i++)
					{
						var propertyMetadata = properties[i];
						FastPropertySetter FastPropertySetter = null;
						for (var j = 0; j < FastPropertySetters.Length; j++)
						{
							if (string.Equals(
								propertyMetadata.PropertyName,
								FastPropertySetters[j].Property.Name,
								StringComparison.Ordinal))
							{
								FastPropertySetter = FastPropertySetters[j];
								break;
							}
						}

						Debug.Assert(FastPropertySetter != null);
						_properties[i] = CreateExplorerForProperty(propertyMetadata, FastPropertySetter);
					}
				}
		...
		public ModelExplorer GetExplorerForProperty(string name, object model)
		{
			...
			return new ModelExplorer(_metadataProvider, this, propertyMetadata, model: model);
		}
			...
			FastPropertySetter FastPropertySetter)
		{
			...
			{
				return c == null ? null : FastPropertySetter.GetValue(c);
			});
